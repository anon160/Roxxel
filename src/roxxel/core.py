...
def _get_default_sharding():
    ...
    if _DEFAULT_SHARDING is None:
        ...
        devices = jax.devices()
        _DEFAULT_MESH = Mesh(
            mesh_utils.create_device_mesh((len(devices),)), axis_names=("data",)
        )
...
class Roxxel:
    ...
    def __init__(self, filepath="./stream_reservoir.rox"):
        ...
        if isinstance(filepath, list):
            ...
            raise TypeError(
                "filepath must be a string (file/pattern) or a list of strings."
            )
    ...
    def _write_raw(
        ...
        def uniform_block_generator():
            ...
            for item in data_generator:
                if isinstance(item, str):
                    ...
                    raise TypeError(
                        "Data generator items must be strings, raw bytes/bytearrays, or numpy arrays."
                    )
        ...
        self._write_orchestrator(
            uniform_block_generator(),
            max_shard_bytes,
            lambda: detected_dtype or "uint8",
        )

    def _write_orchestrator(
        self, block_stream, max_shard_bytes=None, get_dtype=lambda: "uint8"
    ):
            ...
            if last_shard_size < max_shard_bytes:
                ...
                if last_shard_size >= 32:
                    with open(current_shard_path, "rb") as f:
                        ...
                        total_records, raw_data_size, dtype_bytes, file_signature = (
                            struct.unpack("<qq8s8s", footer_block)
                        )

                    if file_signature == b"ROXXEL02":
                        with open(current_shard_path, "rb") as f:
                            f.seek(raw_data_size)
                            end_offsets = np.fromfile(
                                f, dtype="<i8", count=total_records
                            ).tolist()
        ...
        try:
            for block_bytes in block_stream:
                ...
                estimated_size = (
                    current_offset + payload_size + (len(end_offsets) + 1) * 8 + 32
                )
                if estimated_size > max_shard_bytes and len(end_offsets) > 0:
                    f_out.close()
                    self._finalize_shard(
                        current_shard_path, end_offsets, current_offset, get_dtype()
                    )

                    shard_idx += 1
                    current_shard_path = f"{base_name}_{shard_idx:04d}.rox"
                    print(
                        f"📦 Shard limit reached. Creating new shard: {current_shard_path}"
                    )
        ...
        if os.path.exists(path):
            ...
            if total_file_bytes >= 32:
                ...
                if file_signature == b"ROXXEL02":
                    ...
                    print(
                        "⚠️ Invalid signature in existing archive. Overwriting/starting fresh..."
                    )
                    end_offsets = []
                    raw_data_size = 0
            else:
    ...
    def _finalize_shard(self, path, end_offsets, raw_data_size, dtype="uint8"):
        ...
        with open(path, "ab") as f:
            ...
            footer = struct.pack(
                "<qq8s8s", total_records, raw_data_size, dtype_bytes, b"ROXXEL02"
            )
            f.write(footer)
        print(
            f"✅ Finalized shard {os.path.basename(path)} - Records: {total_records}, Data Bytes: {raw_data_size}, Dtype: {dtype}"
        )
    ...
    def open(self):
        ...
        if len(self.filepaths) == 0:
            raise FileNotFoundError(
                "No matching files found for the specified dataset path/pattern."
            )
        ...
        try:
            for path in resolved_paths:
                ...
                if total_file_bytes < 32:
                    raise ValueError(
                        f"Corrupted shard {path}: size is less than 32-byte footer size."
                    )
                ...
                raw_data = np.memmap(
                    f_handle, dtype=np.uint8, mode="r", offset=0, shape=(raw_data_size,)
                )
    ...
    def stream(
        ...
        if mix_datasets and weights:
            ...
            if set(all_datasets.keys()) != set(weights.keys()):
                raise ValueError(
                    "mix_datasets/self and weights must have matching keys."
                )
            ...
            current_phase_steps = (
                start_step - sum(p[0] for p in completed_phases)
                if completed_phases
                else start_step
            )
            if current_phase_steps > 0:
                phases_to_simulate.append(
                    (current_phase_steps, batch_size, seq_len, True)
                )
            ...
            for p_steps, p_batch_size, p_seq_len, is_current in phases_to_simulate:
                ...
                for t in range(p_steps):
                    active_names = [
                        name
                        for name in names
                        if rem_bytes[name] >= p_bytes_per_step[name]
                    ]
                    if not active_names:
                        break
                    ...
                    if len(active_names) > 1:
                        active_weights = np.array(
                            [weights[name] for name in active_names], dtype=np.float64
                        )
                ...
                if is_current:
                    ...
                    for name in names:
                        local_completed_phases[name].append(
                            (phase_counts[name], p_batch_size, p_seq_len)
                        )
            ...
            # Count total available steps without materializing the choice list
            rng_state_snapshot = rng_sim.bit_generator.state
            rem_bytes_snapshot = dict(rem_bytes)
            step_limit = total_steps if total_steps is not None else float("inf")
            counted_steps = 0

            while counted_steps < step_limit:
                active_names = [
                    name
                    for name in names
                    if rem_bytes[name] >= cur_bytes_per_step[name]
                ]
                if not active_names:
                    break
                if len(active_names) > 1:
                ...
                counted_steps += 1
            total_steps = counted_steps

            # 6. Lazy generator — replays the same RNG sequence without storing all choices
            def mix_generator():
                rng_replay = np.random.default_rng(0)
                rng_replay.bit_generator.state = rng_state_snapshot
                rem = dict(rem_bytes_snapshot)
                for _ in range(total_steps):
                    active_names = [
                        name for name in names if rem[name] >= cur_bytes_per_step[name]
                    ]
                    if not active_names:
                        return
                    if len(active_names) > 1:
                        ...
                        if sum_w <= 0:
                            return
                        active_probs = active_weights / sum_w
                        chosen_name = rng_replay.choice(active_names, p=active_probs)
                    else:
                        chosen_name = active_names[0]
                    rem[chosen_name] -= cur_bytes_per_step[chosen_name]
                    try:
        ...
        if completed_phases:
            for p_steps, p_batch_size, p_seq_len in completed_phases:
                if start_step < current_step_accumulator + p_steps:
                    ...
                    total_bytes_to_skip += (
                        steps_in_this_phase * p_batch_size * p_seq_len * element_size
                    )
                    ...
                    total_bytes_to_skip += (
                        p_steps * p_batch_size * p_seq_len * element_size
                    )
        ...
        consumed_blocks, remainder_bytes = divmod(
            total_bytes_to_skip, compile_block_size
        )

        if consumed_blocks > 0:
            global_indices = global_indices[consumed_blocks:].copy()
            print(
                f"⏭️ Roxxel instantly jumped past {consumed_blocks} blocks. Resuming at global step {start_step}."
            )
        ...
        batch_buffer = np.empty(total_bytes_per_batch, dtype=np.uint8)

        def batch_generator():
            record_ptr = 0
            leftover = None  # (memmap_view, byte_offset) — view-based, zero-copy

            # Pre-fill leftover from the initial remainder_bytes skip
            if remainder_bytes > 0 and record_ptr < len(global_indices):
                idx = global_indices[record_ptr]
                raw_slice = self[int(idx)]
                if remainder_bytes < len(raw_slice):
                    leftover = (raw_slice, int(remainder_bytes))
                record_ptr += 1
            ...
            for _ in range(total_steps):
                ...
                if leftover is not None:
                    view, start = leftover
                    available = len(view) - start
                    n = (
                        available
                        if available < total_bytes_per_batch
                        else total_bytes_per_batch
                    )
                    batch_buffer[:n] = view[start : start + n]
                    offset = n
                    if start + n < len(view):
                        leftover = (view, start + n)
                    else:
                        leftover = None

                while offset < total_bytes_per_batch:
                    if record_ptr >= len(global_indices):
                        return
                    idx = global_indices[record_ptr]
                    raw_slice = self[int(idx)]
                    remaining = total_bytes_per_batch - offset
                    n = len(raw_slice) if len(raw_slice) < remaining else remaining
                    batch_buffer[offset : offset + n] = raw_slice[:n]
                    offset += n
                    if n < len(raw_slice):
                        leftover = (raw_slice, n)
                    record_ptr += 1

                # Interpret the byte buffer as the storage dtype, then convert to training dtype.
                # Always call .astype() to ensure a fresh copy so the reused batch_buffer
                # cannot corrupt previously yielded batches on CPU (where JAX shares memory).
                try:
                    flat_tokens = batch_buffer.view(native_dtype).astype(dtype)
                except ValueError:
                    flat_tokens = np.frombuffer(
                        batch_buffer.tobytes(), dtype=native_dtype
                    ).astype(dtype)
