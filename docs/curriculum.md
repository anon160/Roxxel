...
Roxxel provides first-class support for multi-phase curriculum schedules and dataset blending natively using the `Curriculum` abstraction.
...
```python
from roxxel import Roxxel, Curriculum
...
# Format: list of dicts specifying steps (or epochs), batch_size, seq_len, and optional weights
phases = [
...
Each phase configuration dictionary encapsulates:
- `steps` (or `epochs`): Target training steps or epochs for this specific profile.
- `batch_size`: The batch size to yield to device accelerators.
- `seq_len`: The sequence length of each sample in the batch.
- `weights` (optional): Blending ratios (summing to 1.0) defining how samples are mixed between datasets.
